...
def train(chord_tensor, category_tensor, category_cls_tensor, event_tensor, model, model_d, optimizer, optimizer_d,
          lambda_d, lambda_kl, is_ordinal):
    """

    :param chord_tensor: [num_chord]
    :param category_tensor: [category_dim]
    ...
    loss_d = model_d.calc_loss(output, category_cls_tensor, is_discriminator=True, is_ordinal=is_ordinal)
    loss_d.backward()
    ...
    # train auto-encoder
    optimizer.zero_grad()
    loss_recon, _, lv, acc, distribution = model(event_tensor, chord_tensor, category_tensor)
    dis_out = model_d(lv)
    loss_d_gen = model_d.calc_loss(dis_out, category_cls_tensor, is_discriminator=False, is_ordinal=is_ordinal)
    ...
    with torch.no_grad():
        for i in range(0, len(keys_test), batch_size):
            ...
            if len(key_indices) < batch_size:
                continue
            if use_chord_vector:
                chord_tensor = to_float_tensor([a_chord_test[key] for key in key_indices], device=device)
            else:
                chord_tensor = to_tensor([a_chord_test[key] for key in key_indices], device=device)
            style_tensor = to_float_tensor([[a_category_test[attr + '/' + key] for attr in attrs]
                                            for key in key_indices], device=device).reshape(batch_size, -1)
            style_cls_tensor = to_tensor([[a_category_cls_test[attr + '/' + key] for attr in attrs]
                                            for key in key_indices], device=device).reshape(batch_size, -1)
            event_tensor = to_tensor([a_event_test[key] for key in key_indices], device=device)
...
def train_iters(model, model_d, optimizer, optimizer_d, cfg, device, print_every=500, model_save_every=100,
    ...
    if use_chord_vector:
        a_chord = h5py.File(cfg['data']['chord_a'], 'r')
        a_chord_test = h5py.File(cfg['data']['chord_a_test'], 'r')
    else:
        a_chord = h5py.File(cfg['data']['chord'], 'r')
        a_chord_test = h5py.File(cfg['data']['chord_test'], 'r')

    #additional
    a_category = h5py.File(cfg['data']['attr_cls'], 'r')
    a_category_cls = h5py.File(cfg['data']['attr_cls'], 'r')
    a_event = h5py.File(cfg['data']['event'], 'r')
    with open(cfg['data']['keys'], 'rb') as f:
        keys = pickle.load(f)

    a_category_test = h5py.File(cfg['data']['attr_test'], 'r')
    a_category_cls_test = h5py.File(cfg['data']['attr_cls_test'], 'r')
    a_event_test = h5py.File(cfg['data']['event_test'], 'r')
    ...
    for iter in range(start_iter + 1, cfg['n_iters'] + 1):
        ...
        if use_chord_vector:
            chord_tensor = to_float_tensor([a_chord[key] for key in key_indices], device=device)
        else:
            chord_tensor = to_tensor([a_chord[key] for key in key_indices], device=device)
        category_tensor = to_float_tensor([[a_category[attr + '/' + key] for attr in cfg['attr']]
                                        for key in key_indices], device=device).reshape(cfg['batch_size'], -1)
        category_cls_tensor = to_tensor([[a_category_cls[attr + '/' + key] for attr in cfg['attr']]
                                        for key in key_indices], device=device).reshape(cfg['batch_size'], -1)
        event_tensor = to_tensor([a_event[key] for key in key_indices], device=device)

        loss, loss_d, loss_kl, acc = train(chord_tensor, category_tensor, category_cls_tensor, event_tensor, model, model_d,
        ...
        if iter % print_every == 0:
            ...
            # Test
            val_loss, val_loss_d, val_loss_kl, acc = test(a_chord_test, a_category_test, a_category_cls_test, a_event_test,
...
def run(args):
    ...
    cfg = generate_cfg_adversarial(env_var, args, output_dir, checkpoint)
    print(cfg)
