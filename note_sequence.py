...
class Note(object):
    def __init__(self, program, start_time, end_time, pitch, velocity, is_drum):
    ...
    def __repr__(self):
        return "start: {}, end: {}, pitch: {}, qstart: {}, qend: {}\n".format(
         self.start_time, self.end_time, self.pitch,  self.quantized_start_step,
            self.quantized_end_step)


class PitchBend(object):
    def __init__(self, program, time, pitch, is_drum):
        self.program = program
        self.time = time
        self.pitch = pitch
        self.is_drum = is_drum
...
class ControlChange(object):
    def __init__(self, program, time, control_number, control_value, is_drum):
        self.program = program
        self.time = time
...
class PerformanceEvent(object):
    ...
    def __init__(self, event_type, event_value, ):
        if event_type in (PerformanceEvent.NOTE_ON, PerformanceEvent.NOTE_OFF):
            ...
            if not 0 <= event_value:
                raise ValueError('Invalid time shift value: %s' % event_value)
        elif event_type in [PerformanceEvent.ENDS_OF_SEQ, PerformanceEvent.PADDING]:
            pass
        else:
            raise ValueError('Invalid event type: %s' % event_type)
